...
function SidebarScreen() {
  const items = [
    ...
    { icon: Smartphone, label: "Inventory", active: true },
  ...
  return (
    <div className="flex h-full bg-white">
      <div className="w-28 bg-slate-900 h-full py-3 px-2 flex flex-col gap-0.5 shrink-0">
        <div className="text-[7px] font-black text-blue-400 tracking-widest uppercase mb-2 px-1">
          StockFlow
        </div>
      ...
      <div className="flex-1 bg-slate-50 p-2 overflow-hidden">
        <div className="text-[8px] font-bold text-slate-900 mb-1.5">Inventory</div>
        {[
          { name: "iPhone 15 Pro", status: "In Stock", price: "₹89,000", c: "text-blue-600 bg-blue-50" },
          { name: "Samsung S24+", status: "Pending", price: "₹72,000", c: "text-amber-600 bg-amber-50" },
          { name: "Redmi Note 13", status: "Sold", price: "₹18,500", c: "text-emerald-600 bg-emerald-50" },
        ].map(({ name, status, price, c }) => (
          <div key={name} className="bg-white rounded mb-1 p-1.5 flex items-center justify-between shadow-sm">
            <div>
              <div className="text-[7px] font-bold text-slate-900">{name}</div>
              <span className={`text-[5.5px] font-bold px-1 py-0.5 rounded-full ${c}`}>{status}</span>
            </div>
            <div className="text-[7px] font-bold text-slate-700">{price}</div>
          </div>
        ))}
      </div>
    </div>
...
function DashboardScreen() {
  return (
    <div className="flex flex-col bg-slate-100 h-full overflow-hidden">
      <div className="bg-blue-600 px-3 pt-2.5 pb-3 text-white shrink-0">
        <p className="text-[6px] uppercase tracking-widest text-white/60 mb-0.5">Net Cash Flow</p>
        <p className="text-[18px] font-black leading-tight">₹1,24,500</p>
        <div className="flex gap-1.5 mt-1.5">
          <span className="bg-black/20 px-1.5 py-0.5 rounded-md text-[6px] text-emerald-300">↑ ₹45k Sales</span>
          <span className="bg-black/20 px-1.5 py-0.5 rounded-md text-[6px] text-white/70">₹82k Stock</span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-1.5 p-2 flex-1">
        {[
          { label: "Purchases", val: "₹1.2L", sub: "42 orders", b: "border-b-rose-500" },
          { label: "Sales", val: "₹45,000", sub: "38 orders", b: "border-b-emerald-500" },
          { label: "Avg Profit", val: "₹1,200", sub: "per device", b: "border-b-blue-500" },
          { label: "In Stock", val: "38 units", sub: "4 pending", b: "border-b-amber-500" },
        ].map(({ label, val, sub, b }) => (
          <div key={label} className={`bg-white rounded-lg px-2 py-1.5 shadow-sm border-b-2 ${b}`}>
            <p className="text-[5.5px] uppercase tracking-wider text-slate-400 font-bold">{label}</p>
            <p className="text-[9px] font-black text-slate-900 mt-0.5">{val}</p>
            <p className="text-[5.5px] text-slate-400">{sub}</p>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="flex flex-col bg-slate-50 h-full">
      <div className="p-2 bg-white border-b border-slate-100 shrink-0">
        <div className="bg-slate-100 rounded-lg px-2 py-1 flex items-center gap-1.5 mb-1.5">
          <div className="size-2 rounded-full border border-slate-400" />
          <span className="text-[7px] text-slate-400">Search IMEI, model, brand…</span>
        </div>
        <div className="flex gap-1">
          {["All", "In Stock", "Pending", "Sold"].map((t, i) => (
            <span key={t} className={`text-[6px] font-bold px-1.5 py-0.5 rounded-full ${i === 0 ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-500"}`}>{t}</span>
          ))}
        </div>
      </div>
      <div className="flex-1 overflow-hidden p-1.5 flex flex-col gap-1">
        {[
          { name: "iPhone 15 Pro Max", brand: "Apple", imei: "354213xxxxxx", c: "bg-blue-50 text-blue-600", label: "In Stock", price: "₹89,000" },
          { name: "Samsung Galaxy S24+", brand: "Samsung", imei: "490217xxxxxx", c: "bg-amber-50 text-amber-600", label: "Pending", price: "₹72,000" },
          { name: "Redmi Note 13 Pro", brand: "Xiaomi", imei: "352109xxxxxx", c: "bg-emerald-50 text-emerald-600", label: "Sold", price: "₹18,500" },
          { name: "Vivo V30 Pro", brand: "Vivo", imei: "864201xxxxxx", c: "bg-blue-50 text-blue-600", label: "In Stock", price: "₹38,000" },
        ].map(({ name, brand, imei, c, label, price }) => (
          <div key={name} className="bg-white rounded-lg p-1.5 flex items-center gap-1.5 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[7px] font-bold text-slate-900 truncate">{name}</div>
              <div className="text-[5.5px] text-slate-400 truncate">{brand} · {imei}</div>
            </div>
            <div className="flex flex-col items-end gap-0.5 shrink-0">
              <span className={`text-[5.5px] font-bold px-1 py-0.5 rounded-full ${c}`}>{label}</span>
              <span className="text-[6px] font-bold text-slate-700">{price}</span>
            </div>
          </div>
        ))}
      </div>
...
function PurchaseOrdersScreen() {
  return (
    <div className="flex flex-col bg-slate-50 h-full">
      <div className="bg-white px-2.5 pt-2 pb-1.5 border-b border-slate-100 flex items-center justify-between shrink-0">
        <div>
          <div className="text-[8px] font-black text-slate-900">Purchase Orders</div>
          <div className="text-[6px] text-slate-400">4 orders · ₹4.63L total</div>
        </div>
        <div className="bg-blue-600 text-white text-[7px] font-bold px-1.5 py-0.5 rounded-lg">+ New PO</div>
      </div>
      <div className="flex-1 overflow-hidden p-1.5 flex flex-col gap-1">
        {[
          ...
          <div key={id} className="bg-white rounded-lg p-2 shadow-sm flex items-center justify-between">
            <div>
              <div className="flex items-center gap-1 mb-0.5">
                <span className="text-[7px] font-black text-blue-600">{id}</span>
                <span className={`text-[5.5px] font-bold px-1 py-0.5 rounded-full ${c}`}>{status}</span>
              </div>
              <div className="text-[7px] text-slate-600 font-medium">{sup}</div>
              <div className="text-[5.5px] text-slate-400">{items} items</div>
            </div>
            <div className="text-right">
              <div className="text-[8px] font-black text-slate-900">{amount}</div>
              <ChevronRight size={8} className="text-slate-300 ml-auto mt-0.5" />
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="flex flex-col bg-slate-50 h-full overflow-hidden">
      <div className="bg-white px-2.5 py-2 border-b border-slate-100 shrink-0">
        <div className="flex items-center justify-between mb-0.5">
          <span className="text-[8px] font-black text-slate-900">PO-042</span>
          <span className="bg-emerald-50 text-emerald-700 text-[6px] font-bold px-1.5 py-0.5 rounded-full">✓ Certified</span>
        </div>
        <div className="text-[6.5px] text-slate-500">Raju Mobile Traders · 12 May 2026</div>
      </div>
      <div className="flex-1 overflow-hidden p-2 flex flex-col gap-1">
        <div className="text-[6px] uppercase text-slate-400 font-bold tracking-wider">Items (3)</div>
        {[
          { name: "iPhone 15 Pro", qty: 2, price: "₹89,000" },
          { name: "Samsung S24+", qty: 3, price: "₹72,000" },
          { name: "Vivo V30 Pro", qty: 3, price: "₹38,000" },
        ].map(({ name, qty, price }) => (
          <div key={name} className="bg-white rounded-lg p-1.5 flex items-center justify-between shadow-sm">
            <div>
              <div className="text-[7px] font-bold text-slate-900">{name}</div>
              <div className="text-[5.5px] text-slate-400">×{qty} units</div>
            </div>
            <div className="text-[7px] font-bold text-slate-700">{price}</div>
          </div>
        ))}
        <div className="bg-white rounded-lg p-1.5 shadow-sm">
          <div className="text-[6px] uppercase text-slate-400 font-bold tracking-wider mb-1">Payment</div>
          {[
            ...
            <div key={label} className="flex justify-between mt-0.5">
              ...
              <span className={`text-[7px] font-bold ${c}`}>{val}</span>
            </div>
          ))}
        </div>
      </div>
...
function SalesOrdersScreen() {
  return (
    <div className="flex flex-col bg-slate-50 h-full">
      <div className="bg-white px-2.5 pt-2 pb-1.5 border-b border-slate-100 flex items-center justify-between shrink-0">
        <div>
          <div className="text-[8px] font-black text-slate-900">Sales Orders</div>
          <div className="text-[6px] text-slate-400">4 orders · ₹1.85L this month</div>
        </div>
        <div className="bg-emerald-600 text-white text-[7px] font-bold px-1.5 py-0.5 rounded-lg">+ New SO</div>
      </div>
      <div className="flex-1 overflow-hidden p-1.5 flex flex-col gap-1">
        {[
          ...
          <div key={id} className="bg-white rounded-lg p-2 shadow-sm flex items-center justify-between">
            <div>
              ...
              <div className="text-[7px] text-slate-600 font-medium">{cust}</div>
            </div>
            <div className="text-right">
              <div className="text-[8px] font-black text-slate-900">{amount}</div>
              <ChevronRight size={8} className="text-slate-300 ml-auto mt-0.5" />
...
function LedgerScreen() {
  return (
    <div className="flex flex-col bg-slate-50 h-full">
      <div className="bg-white px-2.5 py-2 border-b border-slate-100 shrink-0">
        <div className="text-[6px] text-slate-400 uppercase tracking-wider">Available Balance</div>
        <div className="text-[16px] font-black text-slate-900 leading-tight">₹2,34,500</div>
        <div className="flex gap-1.5 mt-1.5">
          <div className="flex-1 bg-emerald-50 rounded-lg p-1">
            <div className="text-[5.5px] text-emerald-700 font-bold uppercase">Total In</div>
            <div className="text-[8px] font-black text-emerald-700">₹4,80,000</div>
          </div>
          <div className="flex-1 bg-rose-50 rounded-lg p-1">
            <div className="text-[5.5px] text-rose-700 font-bold uppercase">Total Out</div>
            <div className="text-[8px] font-black text-rose-700">₹2,45,500</div>
          </div>
        </div>
      </div>
      <div className="flex-1 overflow-hidden p-1.5 flex flex-col gap-0.5">
        {[
          ...
          <div key={label} className="bg-white rounded-lg px-2 py-1.5 flex items-center justify-between shadow-sm">
            <div>
              <div className="text-[7px] font-bold text-slate-900">{label}</div>
              <div className="text-[5.5px] text-slate-400">{date}</div>
            </div>
            <span className={`text-[8px] font-black ${c}`}>{amount}</span>
          </div>
        ))}
      </div>
    </div>
...
function AnalyticsScreen() {
  ...
  return (
    <div className="flex flex-col bg-slate-50 h-full">
      <div className="bg-white px-2.5 py-2 border-b border-slate-100 shrink-0">
        <div className="text-[8px] font-black text-slate-900">Analytics</div>
        <div className="text-[6px] text-slate-400">Jan – May 2026</div>
      </div>
      <div className="grid grid-cols-3 gap-1 p-1.5 shrink-0">
        {[
          ...
          <div key={label} className="bg-white rounded-lg p-1.5 shadow-sm text-center">
            <div className={`text-[7px] font-black ${c}`}>{val}</div>
            <div className="text-[5.5px] text-slate-400 font-bold uppercase mt-0.5">{label}</div>
          </div>
        ))}
      </div>
      <div className="flex-1 flex items-end gap-1.5 px-3 pb-2 pt-1">
        {bars.map(({ month, revenue, cogs }) => (
          <div key={month} className="flex-1 flex flex-col items-center gap-0.5">
            ...
            <span className="text-[5.5px] text-slate-400 font-medium">{month}</span>
          </div>
        ))}
      </div>
      <div className="flex gap-3 justify-center pb-1.5 shrink-0">
        <div className="flex items-center gap-0.5">
          <div className="size-1.5 rounded-full bg-blue-400" />
          <span className="text-[5.5px] text-slate-500">Revenue</span>
        </div>
        <div className="flex items-center gap-0.5">
          <div className="size-1.5 rounded-full bg-rose-300" />
          <span className="text-[5.5px] text-slate-500">COGS</span>
        </div>
      </div>
    </div>
  );
...
function CustomersScreen() {
  return (
    <div className="flex flex-col bg-slate-50 h-full">
      <div className="bg-white px-2.5 pt-2 pb-1.5 border-b border-slate-100 shrink-0">
        <div className="text-[8px] font-black text-slate-900 mb-1">Customers</div>
      ...
      <div className="flex-1 overflow-hidden p-1.5 flex flex-col gap-1">
        {[
          ...
          <div key={name} className="bg-white rounded-lg p-1.5 flex items-center gap-1.5 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[7px] font-bold text-slate-900">{name}</div>
              <div className="text-[5.5px] text-slate-400 truncate">{phone} · {orders} orders</div>
            </div>
            <div className="text-right shrink-0">
              <div className="text-[7px] font-bold text-slate-900">{balance}</div>
              {due && <div className="text-[5.5px] text-rose-500 font-bold">Due</div>}
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="flex flex-col bg-slate-50 h-full overflow-hidden">
      <div className="bg-white px-2.5 py-2 border-b border-slate-100 shrink-0">
        <div className="flex items-center gap-2 mb-1.5">
          ...
          <div>
            <div className="text-[8px] font-black text-slate-900">Ahmed Khan</div>
            <div className="text-[6px] text-slate-400">+91 98765 43210 · 5 orders</div>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-1">
          {[
            ...
            <div key={label} className="bg-slate-50 rounded-lg p-1 text-center border border-slate-100">
              <div className={`text-[7px] font-black ${c}`}>{val}</div>
              <div className="text-[5px] text-slate-400 font-bold uppercase mt-0.5 leading-tight">{label}</div>
            </div>
          ))}
        </div>
      </div>
      <div className="flex-1 overflow-hidden p-1.5 flex flex-col gap-0.5">
        <div className="text-[6px] uppercase text-slate-400 font-bold tracking-wider mb-0.5">Order History</div>
        {[
          ...
          <div key={id} className="bg-white rounded-lg px-2 py-1.5 flex items-center justify-between shadow-sm">
            <div>
              <div className="text-[7px] font-bold text-emerald-700">{id}</div>
              <div className="text-[5.5px] text-slate-400">{date}</div>
            </div>
            <div className="flex items-center gap-1">
              <div className="text-[7px] font-bold text-slate-900">{amount}</div>
              <span className={`text-[5.5px] font-bold px-1 py-0.5 rounded-full ${status === "Paid" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
                {status}
              </span>
            </div>
          </div>
