...
import { setTeam, updateMemberRole } from "@/features/tenant/slice";
...
export default function ManageTeam() {
  ...
  useEffect(() => {
    async function fetchTeam() {
      ...
      // If we already have members and aren't refreshing, we could skip, 
      // but for team mgmt we usually want the latest from DB if online.
      setIsLoading(true);
      try {
  ...
  const handleRoleChange = (userId: string, newRole: string) => {
    if (!userId) return;
    // 1. Optimistic UI update in Redux
    dispatch(updateMemberRole({ id: userId, role: newRole }));
    
    // 2. Queue for offline sync
    dispatch(queueAction({
      type: "tenant/updateMemberRole",
      payload: { id: userId, role: newRole }
    }));
    
    toast.success("Role update queued.");
  };
        ...
        <main className="flex-1 p-4">
          ...
          <div>
            <h3 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-3 pl-1">
              Current Members ({profiles?.length || 0})
            </h3>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm dark:shadow-black/20 border border-slate-100 dark:border-slate-800 overflow-hidden divide-y divide-slate-50 dark:divide-slate-800">
              {isLoading && profiles.length === 0 ? (
                ...
                profiles.map((profile) => (
                  <div key={profile?.id || Math.random()} className="p-4 flex items-center gap-4">
                    <div className="size-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center shrink-0 uppercase font-black text-slate-400 dark:text-slate-500 border border-slate-200 dark:border-slate-700">
                    ...
                    <div>
                      {isAdmin && profile?.id && profile.id !== session?.user.id ? (
                        <Select
                          defaultValue={profile?.role || "associate"}
                          onValueChange={(val) =>
                        ...
                        getRoleBadge(profile?.role || "associate")
                      )}
                    </div>
                  </div>
                ))
