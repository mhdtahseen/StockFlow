...
import { Package, Search, ChevronRight, CheckSquare } from "lucide-react";
...
export default function PurchaseOrders() {
  ...
  const multi = useMultiSelect(purchaseOrders);
  ...
  const filteredOrders = useMemo(() => {
    return purchaseOrders
      ...
      .filter((o) => {
        ...
        const matchesImei = o.items.some((item: PurchaseOrderItem) => {
  ...
  const getChannelColor = (channel: string) => {
    switch (channel) {
      case 'DIRECT': return "text-purple-600 dark:text-purple-400 font-black";
      case 'PLATFORM': return "text-teal-600 dark:text-teal-400 font-black";
      case 'INTER_TENANT': return "text-indigo-600 dark:text-indigo-400 font-black";
      ...
      <div className="sticky top-0 z-20 pt-1 pb-1">
        <div className="bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-200/50 dark:border-slate-800/50 px-4 py-3 space-y-3">
          ...
          <div className="relative group">
            ...
            <input
              type="text"
              placeholder="Search by supplier, ID or IMEI..."
              value={search}
          ...
          {filteredOrders.length === 0 ? (
            ...
            >
              ...
              <p className="text-xs mt-1.5 text-slate-500 dark:text-slate-500 max-w-[200px] leading-relaxed italic">
                {search || activeFilter !== 'ALL' ? "Adjust your search to find what you're looking for." : "Start by creating your first purchase order."}
              </p>
            </motion.div>
          ) : (
                  ...
                  <div className="flex items-start justify-between relative z-10">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2.5 mb-2.5">
                        ...
                        <span className={clsx("text-[9px] uppercase tracking-widest opacity-60", getChannelColor(order.acquisitionChannel))}>
                          {order.acquisitionChannel}
                        </span>
                      </div>
                      
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="font-mono text-[15px] font-black text-slate-900 dark:text-slate-100 leading-none">
                      ...
                      <div className="flex items-center gap-3 text-[11px] font-semibold text-slate-400 mt-2.5">
                        <span className="flex items-center gap-1.5">
                          <Package size={13} className="opacity-40" />
                          <span className="text-slate-700 dark:text-slate-300">{order.phonesOrdered}</span> {order.phonesOrdered === 1 ? 'device' : 'devices'}
                        </span>
                        {order.phonesReceived > 0 && (
                          <div className="flex items-center gap-1.5">
                            <span className="size-1 rounded-full bg-slate-200 dark:bg-slate-700" />
                            <span className="text-primary-500/90 font-bold">{order.phonesReceived} received</span>
                          </div>
                        )}
                      </div>
                    </div>
