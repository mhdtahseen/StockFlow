  ...
  return (
    <div className="p-4 sm:p-5 w-full space-y-6 pb-32">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" className="rounded-xl border-slate-200 dark:border-slate-800 font-bold gap-2">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4 text-slate-400">
          ...
          <p className="font-bold text-xs uppercase tracking-widest">Hydrating Catalog...</p>
        </div>
      ) : (
        <div className="grid gap-6">
          {plans.map((plan) => (
            <Card key={plan.id} className="rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
              <CardHeader className="bg-slate-50 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800/50">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    {editingId === plan.id ? (
                      <>
                        <Button variant="ghost" size="sm" onClick={cancelEdit} className="rounded-lg h-8 w-8 p-0">
                          <X size={16} />
                        </Button>
                        <Button onClick={handleUpdate} size="sm" className="rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white h-8 px-3 gap-1.5 font-bold">
                          <Save size={16} /> Save
                        </Button>
                      </>
                    ) : (
                      <Button variant="outline" size="sm" onClick={() => handleEdit(plan)} className="rounded-lg border-slate-200 dark:border-slate-800 h-8 px-3 gap-1.5 font-bold">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <Label className="text-xs font-semibold text-slate-500">Monthly (INR)</Label>
                        {editingId === plan.id ? (
                          <Input
                            type="number"
                            value={editForm?.price_monthly}
                            onChange={(e) => setEditForm(prev => prev ? ({ ...prev, price_monthly: Number(e.target.value) }) : null)}
                            className="rounded-lg font-bold h-10"
                          />
                        ) : (
                          <p className="text-xl sm:text-2xl font-bold tabular-nums">₹{plan.price_monthly}</p>
                        )}
                      </div>
                      <div className="space-y-1.5">
                        <Label className="text-xs font-semibold text-slate-500">Yearly (INR)</Label>
                        {editingId === plan.id ? (
                          <Input
                            type="number"
                            value={editForm?.price_yearly}
                            onChange={(e) => setEditForm(prev => prev ? ({ ...prev, price_yearly: Number(e.target.value) }) : null)}
                            className="rounded-lg font-bold h-10"
                          />
                        ) : (
                          <p className="text-xl sm:text-2xl font-bold tabular-nums">₹{plan.price_yearly}</p>
                        )}
                      </div>
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs font-bold text-slate-500">Tier Description</Label>
                      {editingId === plan.id ? (
                        <textarea
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {editForm?.features.map((f, i) => (
                              <Badge key={i} variant="secondary" className="rounded-md py-1 pr-1 gap-1.5 font-bold uppercase text-[9px]">
                                {f}
                                <button onClick={() => toggleFeature(f)} className="hover:text-rose-500">
                                  <X size={10} />
                                </button>
                              </Badge>
                            ))}
