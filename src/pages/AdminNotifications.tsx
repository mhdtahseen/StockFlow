...
export default function AdminNotifications() {
  ...
  const [targetRole, setTargetRole] = useState<Announcement['target_role']>('all');

  // Push Notification Form
  const [pushTitle, setPushTitle] = useState("");
  const [pushMessage, setPushMessage] = useState("");
  const [pushTarget, setPushTarget] = useState<Announcement['target_role']>('all');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    // Special: If it's a notification, trigger the mass insert
    if (displayType === "notification") {
      const { error: broadcastError } = await supabase.rpc("broadcast_system_notification", {
        p_title: title,
        p_message: content,
    ...
    <div className="p-4 sm:p-5 w-full space-y-6 overflow-x-hidden">
      ...
      <Tabs defaultValue="in-app" className="w-full">
        <TabsList className="bg-slate-100 dark:bg-slate-900 p-1 rounded-xl mb-6">
          <TabsTrigger value="in-app" className="rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm">
            <Megaphone className="size-4 mr-2" />
            In-App Broadcast
          </TabsTrigger>
          <TabsTrigger value="push" className="rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
                ...
                <form onSubmit={handleCreate} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Display As</label>
                      <select 
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Type Color</label>
                      <select 
                        className="w-full h-10 px-2 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-xs font-bold"
                        value={type}
                  ...
                  >
                    {isSubmitting ? <Loader2 className="animate-spin size-4" /> : <Send className="size-4 mr-2" />}
                    Broadcast Live
                  </Button>
                </form>
              </div>
                      ...
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            <h4 className="font-bold text-slate-900 dark:text-slate-100 truncate text-sm">{item.title}</h4>
                            <Badge variant="outline" className="text-[10px] font-bold">
                              {item.target_role.toUpperCase()}
                            </Badge>
                            <Badge variant="secondary" className="text-[9px] font-medium uppercase tracking-tighter bg-slate-100 dark:bg-slate-800">
                              {item.display_type}
                            </Badge>
                          </div>
                          <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2">{item.content}</p>
                          <p className="text-[10px] text-slate-400 mt-2 lowercase">Sent {new Date(item.created_at).toLocaleString()}</p>
                        </div>
                        <div className="flex items-center gap-1">
                          ...
                          >
                            {item.is_active ? <Eye className="size-4 text-emerald-500" /> : <EyeOff className="size-4 text-slate-400" />}
                          </Button>
                          <Button 
                            variant="ghost" 
                            size="icon" 
        ...
        <TabsContent value="push" className="outline-none">
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm max-w-2xl mx-auto">
            <div className="text-center mb-8">
              ...
              <h3 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">Push Broadcaster</h3>
              <p className="text-slate-500 text-sm">
                Send a system-level notification to all subscribed Android/PWA devices.
              </p>
            </div>
            ...
            <form onSubmit={handleSendPush} className="space-y-5">
              ...
              <div className="p-4 bg-primary-50 dark:bg-primary-900/10 rounded-lg border border-primary-100 dark:border-primary-900/20">
                <div className="flex gap-3 text-primary-700 dark:text-primary-400">
                  <Info className="size-4 shrink-0 mt-0.5" />
                  <p className="text-[11px] leading-relaxed">
                    This notification will appear even if the app is closed. Drive users back with something quirky!
                  </p>
                </div>
              </div>
              ...
              >
                {isSendingPush ? <Loader2 className="size-5 animate-spin" /> : <><Send className="size-4 mr-2" /> Blast Push Notification</>}
              </Button>
            </form>
          </div>
        </TabsContent>
...
function getTypeIcon(type: string) {
  switch (type) {
    ...
    case 'feature': return <Megaphone className="size-4 text-purple-500" />;
