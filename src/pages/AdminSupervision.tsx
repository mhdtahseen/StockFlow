...
const PLAN_OPTIONS = ["trial", "starter", "pro", "wholesaler", "free"];
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto w-full space-y-8 pb-32">
      <div className="flex items-center justify-between">
        ...
        >
          {isLoading ? <Loader2 className="animate-spin" size={18} /> : "Sync Registry"}
        </Button>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Hydrating Registry...</p>
        </div>
      ) : (
        <div className="grid gap-6">
          {tenants.map((tenant) => (
            <Card key={tenant.id} className={clsx(
              "border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-all",
              !tenant.is_active && "opacity-75 grayscale-[0.5]"
            )}>
              <div className={clsx(
                "h-1.5 w-full",
                tenant.plan === 'wholesaler' ? "bg-emerald-500" :
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 p-5">
                <div className="flex flex-col">
                  <div className="flex items-center gap-3">
                    ...
                    {!tenant.is_active && (
                      <Badge variant="destructive" className="uppercase font-black text-[10px] tracking-tighter">
                        Paused
                      </Badge>
                    )}
                  </div>
                  <CardDescription className="font-mono text-[10px] mt-1 uppercase tracking-wider">
                    ID: {tenant.id}
                  </CardDescription>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex flex-col items-end mr-4">
                    <span className="text-[10px] uppercase font-black text-slate-400 tracking-widest mb-1.5">Assigned Plan</span>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button 
                          variant="secondary" 
                          size="sm" 
                          className="h-9 px-4 rounded-lg font-black uppercase text-[11px] tracking-wide bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700"
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      className={clsx(
                        "rounded-xl",
                        tenant.is_active ? "text-amber-600 hover:text-amber-700 hover:bg-amber-50" : "text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50"
                      )}
                    >
                      {tenant.is_active ? <PauseCircle size={22} /> : <PlayCircle size={22} />}
                    </Button>
                    <Button
                      variant="ghost"
                      size="icon"
                    ...
                    >
                      {expandedTenants.has(tenant.id) ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
                    </Button>
                  </div>
                </div>
              </CardHeader>
                    ...
                    {tenant.profiles && tenant.profiles.length > 0 ? (
                      tenant.profiles.map((user) => (
                        <div key={user.id} className="p-4 flex items-center justify-between group">
                          <div className="flex items-center gap-4">
                            <div className="size-10 rounded-full bg-white dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-400">
                              {user.role === 'admin' ? <Crown size={18} className="text-amber-500" /> : 
                               user.role === 'manager' ? <Shield size={18} className="text-blue-500" /> : 
                               <User size={18} />}
                            </div>
                            <div>
                          ...
                          <div className="flex items-center gap-3">
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button 
                                  variant="outline" 
                                  size="sm" 
                                  className="h-8 px-3 rounded-lg text-[10px] font-black uppercase tracking-tight gap-1.5"
                            ...
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => confirmDelete(user.id, user.full_name, "user")}
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-xl font-black">Destructive Action</DialogTitle>
            <DialogDescription className="text-slate-600 font-medium leading-relaxed">
              Are you sure you want to {deleteType === 'tenant' ? 'remove the complete tenant' : 'remove this user'} <span className="font-black text-rose-600">"{targetName}"</span>?
              {deleteType === 'tenant' && " All data, inventory, and users belonging to this tenant will be purged permanently."}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-4 gap-2">
            <Button variant="outline" onClick={() => setDeleteId(null)} className="rounded-xl font-bold flex-1">
              Abort Change
            </Button>
            <Button 
              onClick={handleDelete}
              className="rounded-xl bg-rose-600 hover:bg-rose-700 text-white font-bold flex-1"
            >
              Confirm Deletion
            </Button>
          </DialogFooter>
