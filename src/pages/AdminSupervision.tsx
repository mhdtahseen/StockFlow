...
export default function AdminSupervision() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Calculate Pulse Metrics (Rough counts for now)
  ...
  const handleSuspend = async (duration: string) => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success("Tenant Suspended", { description: `Locked until ${resumeAt.toLocaleString()}` });
    ...
    <div className="p-4 sm:p-5 w-full space-y-5 pb-32">
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <p className="text-xl sm:text-2xl font-bold mt-1 tracking-tight">{stats.totalTenants}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Profiles</p>
            <p className="text-xl sm:text-2xl font-bold mt-1 tracking-tight">{stats.totalProfiles}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Total Hardware</p>
            <p className="text-xl sm:text-2xl font-bold mt-1 tracking-tight">{stats.totalDevices}</p>
          </div>
          <div className="bg-emerald-50/30 dark:bg-emerald-900/10 p-4 rounded-xl border border-emerald-100 dark:border-emerald-800/20">
            <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">System Pulse</p>
            <p className="text-xl sm:text-2xl font-bold mt-1 tracking-tight text-emerald-600 dark:text-emerald-400">99.8%</p>
          </div>
        </div>
      )}
              ...
              <CardHeader className="flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0 p-4 sm:p-5">
                <div className="flex flex-col">
                  ...
                  {tenant.suspended_until && !tenant.is_active && (
                    <div className="mt-2 text-[10px] font-bold text-rose-500 uppercase tracking-tight">
                      Locked until: {new Date(tenant.suspended_until).toLocaleString()}
                    </div>
                  )}
                </div>
      ...
      <Dialog open={!!suspensionId} onOpenChange={() => setSuspensionId(null)}>
        <DialogContent className="rounded-2xl max-w-sm">
          <DialogHeader>
            <DialogTitle className="font-black">Temporal Suspension</DialogTitle>
            <DialogDescription>
              Select how long access should be revoked for this organization.
            </DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-2 mt-4">
            {["1h", "8h", "24h", "1w", "forever"].map((duration) => (
              <Button
                key={duration}
                variant="outline"
                className="uppercase font-black text-xs h-12 rounded-xl"
          ...
          <DialogFooter className="mt-4">
            <Button variant="ghost" className="w-full font-bold" onClick={() => setSuspensionId(null)}>
              Cancel
            </Button>
          </DialogFooter>
        </DialogContent>
