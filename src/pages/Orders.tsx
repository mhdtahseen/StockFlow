...
export default function Orders() {
  ...
  const filteredTrade = useMemo(() => {
    return saleOrders
      .map((o: SaleOrder) => ({ ...o, customerName: customerMap[o.counterpartyId] || "Unknown" }))
  ...
  const filteredPurchase = useMemo(() => {
    return purchaseOrders
      .map((o: any) => ({ ...o, customerName: customerMap[o.counterpartyId] || "Unknown" }))
      .filter(o => o.customerName.toLowerCase().includes(search.toLowerCase()))
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950">
      <header className="px-4 pt-[calc(1rem+env(safe-area-inset-top,0px))] pb-2 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 sticky top-0 z-10">
        ...
        <div className="flex gap-4 mt-4">
          <button 
            ...
            className={clsx(
              ...
              activeTab === 'TRADE' ? "border-[#064a98] text-[#064a98]" : "border-transparent text-slate-400"
            )}
          >
          ...
          <button 
            ...
            className={clsx(
              "text-[10px] font-black uppercase tracking-widest pb-2 border-b-2 transition-colors",
              activeTab === 'PURCHASE' ? "border-[#064a98] text-[#064a98]" : "border-transparent text-slate-400"
            )}
          >
          ...
          {activeTab === 'TRADE' ? (
            filteredTrade.length === 0 ? (
              <div className="text-center py-12 px-4 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50">
                <p className="text-slate-500 font-semibold mb-2">No trade orders found.</p>
                <p className="text-xs text-slate-400 font-medium">Create a trade order from the device detail screen.</p>
              </div>
            ) : (
              filteredTrade.map((o) => (
                <div
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs font-semibold text-slate-500 mt-0.5">
                      {o.items.length} item{o.items.length !== 1 ? 's' : ''} • {o.orderType}
                    </div>
                  </div>
            ...
            filteredPurchase.length === 0 ? (
              <div className="text-center py-12 px-4 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50">
                <p className="text-slate-500 font-semibold mb-2">No purchase orders found.</p>
                <p className="text-xs text-slate-400 font-medium">Use 'Stock Up' to create new purchase orders.</p>
              </div>
            ) : (
              filteredPurchase.map((o) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="font-bold text-slate-900 dark:text-slate-100 truncate">{o.customerName}</h3>
                    <div className="text-xs font-semibold text-slate-500 mt-0.5">
                      {o.phonesOrdered} {o.phonesOrdered === 1 ? 'device' : 'devices'} • {o.acquisitionChannel}
                    </div>
                  </div>
                  <div className="text-right flex flex-col items-end shrink-0">
                    <span className="font-black text-rose-600 dark:text-rose-400">₹{o.totalAmount.toLocaleString()}</span>
                    {o.status !== 'SETTLED' && o.status !== 'CANCELLED' && (
                      <span className="text-xs font-bold text-rose-500/60 block mt-0.5">Owed: ₹{(o.totalAmount - o.amountPaid).toLocaleString()}</span>
                    )}
                  </div>
                  <ChevronRight size={18} className="text-slate-300 group-hover:text-[#064a98] transition-colors shrink-0 -mr-1" />
                </div>
