...
-- Fix connect_by_trade_code: the notifications INSERT used wrong column names.
-- Actual schema: (id, tenant_id, user_id, type, title, message, reference_id, read_at, created_at)
-- Function was using: body, metadata — neither exists.
...
BEGIN
  ...
  -- Notify the other business — use actual column names: title + message (not body/metadata)
