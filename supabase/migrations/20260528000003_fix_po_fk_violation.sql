-- =============================================================================
-- Fix: Foreign key violation on purchase_order_items -> phones
--
-- The create_purchase_order RPC was attempting to insert into purchase_order_items
-- *before* upserting the corresponding phone into the phones table. Because
-- foreign keys are evaluated immediately by default, this caused an FK violation
-- when adding new devices.
--
-- Fix: Swap the order of operations in the PL/pgSQL loop to process the phone
-- table INSERT/UPDATE first, then process the purchase_order_items INSERT.
-- =============================================================================
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- =========================================================================
    -- CRITICAL FIX: MUST UPSERT/UPDATE PHONE FIRST BEFORE INSERTING INTO
    -- purchase_order_items TO SATISFY FOREIGN KEY CONSTRAINT (phone_id)
    -- =========================================================================
    ...
    -- Now safe to insert purchase_order_items row.
