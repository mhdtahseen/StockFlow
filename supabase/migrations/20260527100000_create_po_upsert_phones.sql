-- ─────────────────────────────────────────────────────────────────────────────
-- AddDevices Atomic Phone Upsert
--
-- Problem: The create_purchase_order RPC (since v2) only does UPDATE phones
-- for existing phone rows. The AddDevices flow needs phones to be INSERTED
-- atomically inside the same transaction — so the entire PO + phone creation
-- is one single Supabase call with no race conditions.
--
-- Fix: When a p_items entry contains a 'phone_status' key (set to 'IN_STOCK'
-- by the AddDevices flow), the RPC will UPSERT the phone row. If no
-- 'phone_status' is present (the inspection flow from PurchaseOrders), the
-- old UPDATE behaviour is preserved unchanged.
--
-- The purchase_order_items.status is now driven by an 'item_status' key in
-- p_items (defaults to 'PENDING_INSPECTION' if absent), allowing AddDevices
-- to mark items as 'ACCEPTED' immediately.
--
-- Also sets phones_received on the purchase_order row at creation time, so
-- AddDevices POs reflect the correct count without a subsequent certify call.
-- ─────────────────────────────────────────────────────────────────────────────
